...
const Item = ({id,name,image,old_price,new_price}) => {
  return (
    <div className="rounded-xl overflow-hidden shadow-lg">
      ...
      <Link to={`/product/${id}`} className="h-12 w-12 bg-white rounded-full flexCenter absolute top-1/2 bottom-1/2 !py-2 z-20 scale-0 group-hover:scale-100 transition-all duration-700"><FaSearch className="hover:rotate-90 scale-125 transition-all duration-200"/></Link>
      ...
      <div className="p-4 overflow-hidden">
        ...
        <div className="bold-16">{new_price}.00</div>
          <div className="bold-16 text-secondary line line-through">{old_price}</div>
        </div>
      </div>
    </div>
  )
